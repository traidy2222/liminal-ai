...
**Provider routing + prompt caching** (defaults below). Default strategy is `AGENT_PROVIDER_STRATEGY=price` (live benchmark winner on DeepSeek V4 Pro: ~$0.0035/2-round turn, 96% cache on round 2). OpenRouter `sort: "price"` picks the cheapest stable reseller; `session_id` (`AGENT_OPENROUTER_SESSIONS=1`) keeps the chat sticky for KV cache affinity. Use `adaptive` for the same price sort plus session epoch bump on upstream 429; use `cache_first` + `AGENT_PROVIDER_ORDER=DeepInfra` only if you want an explicit pin. Run `npm run benchmark:provider-strategies` to re-test. Combined with `AGENT_PROMPT_CACHE=1`, cached prefix tokens bill at the provider cache-read rate on ReAct round 2+.
...
| `AGENT_WORKFLOW_MAX_AGENTS`    | `64`      | Total sub-agent cap per workflow run (1–500)                    |
| `AGENT_WORKFLOW_TIMEOUT_MS`    | `1800000` | Wall-clock cap for one workflow run                             |
| `AGENT_WORKFLOW_MODEL`         | (fast)    | Optional planner/summarizer model slug (empty = fast model)     |

Full design in the **Dynamic workflows** paragraph under the tool-families list in the Architecture section.
...
**Vault** (Obsidian brain — interconnected LLM-wiki/"nexus") — `vault_write`, `vault_read`, `vault_search`, `vault_list`, `vault_links`, `vault_graph`, `vault_delete`, plus the nexus layer: `vault_ingest` (write + auto-cross-link to nearest notes via `vault_embed.ts`/`vault_index.ts`; maintains `index.md` catalog + `log.md` timeline — Karpathy LLM-Wiki pattern, link-density-first), `vault_ingest_entities` (decomposes content into one atomic note PER ENTITY — person/org/place/event/concept — each a dossier with Identity/Current/History/Relationships; **merges** into an existing entity note so dossiers accrete over time, and cross-links co-mentioned entities; optional thin hub note. Fast-model extraction in `vault_entity_extract.ts`, pure merge logic in `vault_entity_merge.ts`, both unit-tested. This is the GraphRAG/Karpathy entity-page pattern — knowledge as a graph, not big notes), `vault_recall` (GraphRAG retrieval: semantic+BM25 entry nodes → 1-hop wikilink/backlink expansion → assembled bodies within a char budget), `vault_lint` (orphan / near-duplicate-contradiction / stale audit; `fix:true` links orphans to nearest neighbor). Pure graph/cross-link/index logic is in `vault_nexus.ts` (unit-tested). `AGENT_VAULT_ENTITY_EXTRACT` (default on) routes the end-of-turn auto-writer through entity decomposition. Vault tools auto-activate on `knowledge`/`research`/`introspection` intents via `intent_tool_families.ts` (previously activation-only under the `balanced` profile — the main reason the vault sat unused). The end-of-turn auto-writer (`AGENT_VAULT_AUTO_WRITE`, default `research`) routes through `vault_ingest` so auto-notes are connected, not orphaned. World-context priming surfaces vault content snippets (not bare titles).
...
**Tool families** (`tool_catalog.ts`): `files_edit`, `shell`, `git`, `tasks`, `memory_advanced`, `web`, `markets`, `code_intel`, `browser`, `captcha`, `vision`, `audio`, `meta`, `reasoning_advanced` (harness-scoped: `decompose_goal`, `verify_contract`, `dispatch_graph`, `query_tool_outputs`, `research_state`), `dynamic_tools`, `external_api`, `vault`, `document`, `agenda_scheduler`, `synthesis`, `independence`, `navigation`, `harness_ui`, `orchestration` (now also `reflect_debate`, `branch_explore`, `branch_evaluate`), `workflow` (root-only: `plan_workflow`, `run_workflow`, `workflow_status`, `query_workflow`). Every registered tool must belong to a family or the always-set — enforced by `tool_catalog.test.ts`.

**Dynamic workflows (ultracode-equivalent)** — `plan_workflow` → `run_workflow` execute a declarative multi-phase `WorkflowSpec` (`core/workflow_spec.ts`): each phase fans out independent sub-agents via `forkChild()` in bounded concurrency waves, every agent result is written to an **out-of-context** store (`core/workflow_store.ts` → `.agent_workflows/<runId>/`, BM25-queryable), and only a fast-model **distilled phase summary** folds back into the parent context — so the model's context holds the final report, not hundreds of intermediate outputs (the same architecture as Claude Code dynamic workflows). The engine (`core/workflow_runtime.ts`) supports adversarial review phases and a verify→iterate gate (`run_tests`/`run_lint`/critic, bounded by `converge.maxIterations` — the self-heal-loop pattern). Tools are **root-only** (in `ORCHESTRATION_TOOL_NAMES`, not recreated for children, so workflows don't nest). `run_workflow` is approval-gated. The harness knows **where workflows are powerful** primarily via the fast-model intent classifier: `TurnInferenceResult.workflowSuitable` (`intent_inference.ts`) is set true when the task fans out into many independent sub-tasks (codebase-wide audits/sweeps, large migrations/refactors, parallel-component builds, cross-checked multi-angle research). That LLM signal is authoritative when the classifier ran; the `detectWorkflowSignal` regex (`core/workflow_spec.ts`) is the **fallback** when intent inference is off / low-confidence (same trust pattern as the reasoning-budget signals). On a match the harness **auto-activates the `workflow` family** (required because lazy loading hides activation-gated tools, otherwise the model falls back to `spawn_agent`), refreshes the protocol, and nudges toward `plan_workflow`→`run_workflow`. The family is also seeded in the `max_autonomy` profile. `query_workflow` retrieves per-agent detail on demand. **Cross-phase context flow:** each phase's full outputs are published to the session `SharedMemoryBus` under `ctx/wf/<runId>/<phaseId>`, and later-phase sub-agents auto-receive them via `ChildAgentConfig.contextBusPrefix` (same mechanism as `spawn_agent`'s dependency context) plus a concise inline digest — so a downstream agent builds on upstream findings without being told to ask. (Sub-agent collaboration generally: `share_agent_context`/`read_agent_context` over the bus, auto-activated on every spawn by `finalizeChildSpawnTools`; `dependsOn` agents auto-receive each dependency's result via `buildUpstreamDependencyContext`.) Distinct axis from `AGENT_EFFORT` (output) and the reasoning budget.
...
**Reasoning-stall breaker.** Because the external surface exposes reasoning as tools (`think`/`reason`/`plan`/`hypothesize`/`breakdown`), the model can spiral — emitting reasoning-only rounds that narrate a planned tool call without ever issuing it. The failure-gated loop detector (`toolCallStreak`) can't catch this (reasoning tools always succeed, and success resets the streak). `AgentHarness.updateReasoningStallState` runs after each dispatched batch: it counts consecutive reasoning-only rounds + accumulated reasoning-arg chars and, via the pure `evaluateReasoningStall` (`reasoning_profile.ts`, budget-aware through `resolveReasoningStallNudgeThresholdChars`; tighter `roundCap` on `toolFirstBias` turns), escalates `none → nudge → suppress`. `suppress` drops the reasoning tools from the next round's tool list (`REASONING_TOOL_NAMES` filter in `runReActLoop`) so the model must call a concrete tool or finalize. Any round that runs an action tool resets all of it.
