...
- **`invalid_scope` on Xero sign-in** — reconnect with **Extended APIs unchecked** first (core accounting only). If that works, enable **Extended APIs** and reconnect again only after enabling Files, Projects, and Payroll on your app at [developer.xero.com](https://developer.xero.com/myapps). Do not request deprecated `accounting.classicexpenses` scopes.
