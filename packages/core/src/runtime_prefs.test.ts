...
// Phase 1 storage split: runtime_prefs now writes to ~/.liminal/runtime_prefs.json
// by default. These tests exercise the legacy workspace-local path so they need
// AGENT_STORAGE_LAYOUT=legacy to keep their per-workspace isolation guarantees.
// (The global path is tested separately via integration tests; the unit-level
// concern here is the (de)serialization roundtrip behavior.)
function withLegacyLayout<T>(fn: () => Promise<T>): Promise<T> {
  const prev = process.env["AGENT_STORAGE_LAYOUT"];
  process.env["AGENT_STORAGE_LAYOUT"] = "legacy";
  return fn().finally(() => {
    if (prev === undefined) delete process.env["AGENT_STORAGE_LAYOUT"];
    else process.env["AGENT_STORAGE_LAYOUT"] = prev;
  });
}

test("runtime prefs save/load roundtrip", async () => {
  await withLegacyLayout(async () => {
