...
export async function linkEnterpriseHostDependencies(enterpriseRoot: string): Promise<void> {
  ...
  const hostCore = path.dirname(require.resolve("@liminal/core/package.json"));
  const hostTools = path.dirname(require.resolve("@liminal/tools/package.json"));
...
export async function loadEnterpriseModule(): Promise<EnterpriseLoadResult> {
  for (const root of resolveEnterpriseRoots()) {
    ...
    try {
      ...
      return { ok: false, reason: `Failed to load enterprise from ${root}: ${message}` };
    }
  }
