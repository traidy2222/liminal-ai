...
test("isModelIncompatibleWithManagedProxy is always false — all slugs use managed upstream", () => {
  assert.equal(isModelIncompatibleWithManagedProxy(OPENROUTER_MODEL_SLUG.FREE_ROUTER), false);
  assert.equal(isModelIncompatibleWithManagedProxy("openrouter/auto"), false);
  assert.equal(isModelIncompatibleWithManagedProxy("nvidia/foo:free"), false);
  assert.equal(isModelIncompatibleWithManagedProxy("nex-agi/nex-v2-pro:free"), false);
...
test("resolveModelForManagedInference keeps user-selected openrouter/free slug", () => {
  assert.equal(
