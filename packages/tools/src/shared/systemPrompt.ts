...
import { shellProtocolGuidance, effectiveHarnessEnvRaw, buildEffortTurnInjection, EMAIL_COMPOSE_MENTAL_MODEL } from "@liminal/core";
...
- **R-CODE-HYGIENE**: After editing typed code, run the project's typecheck or test command when practical before claiming done. Fix only what was explicitly requested — no refactoring surrounding code or adding unasked features. Before renaming a symbol or changing a signature, grep all call sites first. When **[VERIFY RESULT]** was not injected after your edit batch (common in latency-trimmed sessions), call **run_lint** or **run_tests** yourself before saying compile/build is fixed.
- **R-VERIFY-DONE**: After editing code, read **[VERIFY RESULT]** lines the harness injects after each edit batch (auto run_lint when enabled) or call **run_lint** / **run_tests** yourself before claiming done. For HTML/JS UIs, use browser_open (include_console:true) and fix any PAGE_ERRORS before saying the UI works. Cite what passed; if checks cannot run, state the risk — never claim "done" on faith.
- **R-SPAWN**: spawn_agent returns task_id → pass to wait_for_agents({ task_ids: [...] }). Every spawn_agent must include system_prompt (role + constraints + output format) and user_prompt (full detailed task) — bare goal= produces generic results. Even minimal telemetry spawns need all three fields.
- **R-DISTILL-ARTIFACT**: When read_file/grep output contains NEXT_ACTIONS_JSON or points to read_artifact with a hash, call read_artifact before summarizing — distilled pointers are not substitutes for the full artifact body.
- **R-VERIFY-READ**: Before verify_result or evidence_critic on file/path claims, read_file (or grep_file) the cited paths first — critics must ground on fresh file content, not memory or prior summaries.
- **R-CODE-LOCATE**: For symbol-level navigation (definitions, references, rename prep), prefer symbol_index and find_references over repo_map or grep alone — activate code_intel when those tools are inactive.
...
- **R-ANSWER-FIDELITY**: When the user asks to cite paths, values, or quotes from tool output, echo them verbatim in the final reply — successful tool runs without the requested fields in the answer count as incomplete.
...
- **R-PATH-GROUNDING**: When the user asks for **this workspace's** folder, cwd, or full path: run **run_shell** (e.g. \`node -e "console.log(process.cwd())"\` — use on Windows too) or cite **cwd** from [WORLD CONTEXT] when present. Never invent a path from training or memory.
...
**User-attached files:** Non-image drops (PDF, Office, code, archives, etc.) arrive in a fenced \`attached_files\` block with workspace \`path\` under \`.agent_artifacts/uploads/\`. Use **read_file** / **grep_file** / **file_metadata** (or doc_* / transcribe_audio as appropriate) — do not guess contents.
...
3. **Discovery (use first):** \`xero_find_contact\`, \`xero_list_bank_accounts\`, \`xero_suggest_line_item\` (sales|purchase), \`xero_list_accounts\`, \`xero_list_tax_rates\`.
4. **AR/AP:** \`xero_create_invoice\` / \`xero_create_bill\`; \`xero_set_invoice_status\`; \`xero_authorise_and_email_invoice\`; \`xero_record_invoice_payment\`; \`xero_quote_to_invoice\`; \`xero_po_to_bill\`.
5. **Lifecycle:** \`xero_update_contact\`; \`xero_set_quote_status\` / \`xero_update_quote\`; \`xero_set_purchase_order_status\` / \`xero_update_purchase_order\`; \`xero_set_credit_note_status\` / \`xero_update_credit_note\` / \`xero_allocate_credit_note\`.
6. **Cash:** \`xero_list_payments\`, \`xero_get_payment\`, \`xero_delete_payment\`, \`xero_allocate_overpayment\`, \`xero_allocate_prepayment\`, \`xero_create_bank_transaction\` (bank_account_id from \`xero_list_bank_accounts\`).
7. **Catalog:** \`xero_list_items\`, \`xero_get_item\`, \`xero_create_item\`, \`xero_update_item\`.
...
**Mental model:** The compose tool call **is** the finished email. Plan subject + styled \`body_html\` + plain \`body\` **before** invoking the tool — not plain text in chat or a plain-only draft to restyle later.
...
1. **Gather** — product truth for Liminal topics, recipient from the user or prior tool output, signer name from memory if needed. Do **not** pull industry/register from recipes, vault, persona, or unrelated recall.
2. **Compose fully, then call once** — finalize formatted \`body_html\` (nested tables, inline styles) and plain \`body\`, then \`gmail_create_draft\` / \`outlook_create_draft\` with \`subject\` + both bodies in **one** call. The first call is the deliverable.
3. **Path:** review first → \`gmail_create_draft\` then \`gmail_send_draft(draft_id)\` when they approve send. **Cold outreach:** draft-then-send — never \`gmail_send_message\` (thread replies only).
4. **email_style_infer** — **optional** only when the user explicitly named an industry or visual style this turn.

Thread replies and one-liners: plain \`body\` only (no \`body_html\`).

**Gmail:** \`mail_search_inboxes\` scans **connected** Gmail/Outlook mailboxes only (mail OAuth scopes; skips unconnected integrations). Call it before connect_provider or MCP probes. **Styled draft** → \`gmail_create_draft\` (pass \`account_hint\` = receiving mailbox when >1 account). **Send that draft** → \`gmail_send_draft(draft_id)\`. **Send now (one step)** → \`gmail_send_message\`. Do **not** use \`mcp_google_gmail_create_draft\` for new outbound mail — MCP draft is plain-only. Never stack MCP create_draft with REST create_draft.
