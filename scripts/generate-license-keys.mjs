 ...
 * Private key → CONTROL_PLANE_LICENSE_PRIVATE_KEY_PEM in server .env only (never commit).
 * Public key  → packages/core/src/entitlements.ts (VIREON_LICENSE_PUBLIC_KEY_PEM).
 */
...
const entitlementsPath = path.join(root, "packages/core/src/entitlements.ts");

const { publicKey, privateKey } = generateKeyPairSync("ed25519");
const publicKeyPem = publicKey.export({ type: "spki", format: "pem" });
const privateKeyPem = privateKey.export({ type: "pkcs8", format: "pem" });

console.log("=== CONTROL_PLANE_LICENSE_PRIVATE_KEY_PEM (server .env only — never commit) ===\n");
