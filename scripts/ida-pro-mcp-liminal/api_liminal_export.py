"""Liminal extensions for ida-pro-mcp — patched binary export and input metadata.

Installed by: npm run ida:patch-liminal
"""
...
class InputMetadataResult(TypedDict, total=False):
    input_path: str
    idb_path: str
    module: str
    imagebase: str
...
class ApplyPatchesResult(TypedDict, total=False):
    ok: bool
    input_path: str
    output_path: str
    bytes_patched: int
...
def _default_output_path(input_path: str, output_path: str | None) -> str:
    if output_path and output_path.strip():
        return os.path.abspath(output_path.strip())
    base, ext = os.path.splitext(input_path)
    suffix = ext if ext else ".bin"
    return os.path.abspath(f"{base}.patched{suffix}")


def _byte_is_patched(ea: int) -> bool:
...
def _try_ida_loader_export(output_path: str) -> tuple[bool, str]:
    try:
        import ida_loader
    except ImportError:
...
def _manual_apply_patches(input_path: str, output_path: str) -> int:
...
@tool
@idasync
def get_input_metadata() -> InputMetadataResult:
    """Return paths and PE/module metadata for the database's input file."""
    try:
...
@tool
@idasync
def apply_patches_to_input(
    output_path: Annotated[
        str,
        "Absolute path for the patched binary (e.g. C:/work/steam_api64.patched.dll). "
        "Empty = <input>.patched<ext> beside the original.",
    ] = "",
    overwrite: Annotated[bool, "Overwrite output_path if it already exists"] = False,
) -> ApplyPatchesResult:
    """Apply in-IDB byte patches to a copy of the input file (patched PE/DLL/ELF on disk).

    Workflow: patch/patch_asm in IDA → apply_patches_to_input → verify with shell.
    """
    input_path = ida_nalt.get_input_file_path()
    if not input_path:
        raise IDAError("No input file — open a binary with idb_open first.")
    if not os.path.isfile(input_path):
        raise IDAError(f"Input file missing on disk: {input_path}")
